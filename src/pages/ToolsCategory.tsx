...
const validCategories: ToolCategory[] = ["dev", "student", "business", "prompt", "pdf"];

const ToolsCategory = () => {
  const { category } = useParams<{ category: string }>();
  const navigate = useNavigate();

  if (!category || !validCategories.includes(category as ToolCategory)) {
    return (
  ...
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <section className={`${bgMap[category as ToolCategory]} text-white py-12 px-4`}>
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl sm:text-4xl font-bold mb-2">{cat.label}</h1>
          <p className="text-lg text-white/90">{cat.description} — {catTools.length} tools available</p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {catTools.map((tool, i) => {
            ...
            return (
              <div
                key={tool.slug}
                onClick={() => navigate(`/tools/${category}/${tool.slug}`)}
                className="group bg-card border border-border rounded-xl p-5 hover:shadow-lg hover:border-primary/30 transition-all cursor-pointer animate-fade-up flex flex-col"
